# 捕食—被捕食与捕捞仿真 / Predator–Prey & Fishing Simulation
...
基于常微分方程比较自然演化与捕捞干预下食鱼和鲨鱼种群的动态变化。 本仓库强调“问题—方法—代码—结果”的对应关系，适合作为课程作业、算法练习或建模研究的可复现档案。

### 核心方法

- Lotka–Volterra 类方程 / Lotka–Volterra-style equations
- ODE45 数值积分 / numerical integration with ODE45
- 干预情景对照 / intervention scenario comparison

## English

### Positioning

An ODE-based comparison of prey and shark population dynamics with and without harvesting pressure. The repository keeps the problem statement, implementation, and outputs close together so the work can be inspected and reproduced.
...
| --- | --- | --- |
| 输入 / Input | 整理 `初始种群 / Initial State` 及模型所需参数，检查单位、范围和文件位置。 | Prepare the 初始种群 / Initial State and validate units, ranges, and file locations. |
| 处理 / Process | 通过 `ODE 参数 / Parameters` 将原始问题转换为可计算表示。 | Convert the original problem into a computable representation through ODE 参数 / Parameters. |
| 求解 / Solve | 执行 `时间积分 / Integrate`，保留中间结果以便检查。 | Execute 时间积分 / Integrate and retain intermediate artifacts for inspection. |
| 输出 / Output | 生成 `情景对比 / Compare`，并结合约束解释结果。 | Produce 情景对比 / Compare and interpret it together with the constraints. |

## 方法设计 / Method Design

| # | 方法与作用 / Method and role |
| ---: | --- |
| 1 | Lotka–Volterra 类方程 / Lotka–Volterra-style equations |
| 2 | ODE45 数值积分 / numerical integration with ODE45 |
| 3 | 干预情景对照 / intervention scenario comparison |
...
```mermaid
flowchart LR
    A["初始种群 / Initial State"] --> B["ODE 参数 / Parameters"] --> C["时间积分 / Integrate"] --> D["情景对比 / Compare"]
```

## 结果展示 / Results

| <img src="assets/results/09-wubulaomain1-01.png" alt="无捕捞 / Without harvesting" width="100%"><br><sub>无捕捞 / Without harvesting</sub> | <img src="assets/results/10-youbulaomain2-01.png" alt="有捕捞 / With harvesting" width="100%"><br><sub>有捕捞 / With harvesting</sub> |
| --- | --- |


**验证摘要 / Verification summary**

两种情景均已在 MATLAB R2025a 成功积分并绘图。 / Both scenarios were integrated and rendered successfully on MATLAB R2025a.
...
| 路径 / Path | 作用 / Purpose |
| --- | --- |
| `sharkFish1.m` | 无捕捞动力学 / no-harvest dynamics |
| `sharkFish2.m` | 捕捞动力学 / harvested dynamics |
| `wubulaomain1.m` | 无捕捞入口 / baseline runner |
| `youbulaomain2.m` | 捕捞入口 / intervention runner |
...
```text
wubulaomain1; figure; youbulaomain2
```
